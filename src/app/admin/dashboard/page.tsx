...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      // Load users
      const usersResponse = await fetch('http://127.0.0.1:8001/api/auth/admin/users', {
        headers: {
      ...
      // Load beta codes
      const codesResponse = await fetch('http://127.0.0.1:8001/api/auth/admin/beta-codes', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      })
  ...
  const loadMetricsData = async () => {
    try {
      ...
      const metricsResponse = await fetch('http://127.0.0.1:8001/api/admin/metrics/platform')
      ...
      // Load cost summary
      const costResponse = await fetch('http://127.0.0.1:8001/api/admin/costs/summary')
      if (costResponse.ok) {
      ...
      const searchesResponse = await fetch('http://127.0.0.1:8001/api/admin/searches/recent?limit=10')
  ...
  const generateBetaCodes = async () => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8001/api/auth/admin/generate-beta-codes', {
        method: 'POST',
        headers: {
